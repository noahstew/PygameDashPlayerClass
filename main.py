...
class Player:
    ...
    def update(self, keys):
        ...
        # Dash mechanic
        if (keys[pygame.K_w] or keys[pygame.K_UP]) and keys[pygame.K_SPACE]:
            self.y -= DASH_DISTANCE
        if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and keys[pygame.K_SPACE]:
            self.y += DASH_DISTANCE
        if (keys[pygame.K_a] or keys[pygame.K_LEFT]) and keys[pygame.K_SPACE]:
            self.x -= DASH_DISTANCE
        if (keys[pygame.K_d] or keys[pygame.K_RIGHT]) and keys[pygame.K_SPACE]:
            self.x += DASH_DISTANCE
    ...
    def draw(self):
        pygame.draw.circle(DISPLAY, WHITE, (self.x, self.y), PLAYER_SIZE)
